...
class SentenceREDataset(data.Dataset):
    ...
    @staticmethod
    def update_rel_stat(pred_lbl, true_lbl, stats, neg):
        """
            stats: {rel_name: [tp, num_predicted_rel, num_relevant_rel]}
        """
        if pred_lbl != neg:
            if pred_lbl not in stats:
                stats[pred_lbl] = [0, 1, 0]
            else:
                stats[pred_lbl][1] += 1

        if true_lbl != neg:
            if true_lbl not in stats:
                stats[true_lbl] = [0, 0, 1]
            else:
                stats[true_lbl][2] += 1

        if pred_lbl == true_lbl and true_lbl != neg:
            stats[pred_lbl][0] += 1

    @staticmethod
    def get_macro_stats(stats):
        sum_p = 0
        sum_r = 0
        sum_f1 = 0
        for stat in stats.values():
            try:
                p = float(stat[0]) / stat[1]
            except:
                p = 0
            try:
                r = float(stat[0]) / stat[2]
            except:
                r = 0
            try:
                f1 = 2 * p * r / (p + r)
            ...
            sum_r += r
            sum_f1 += f1
        return sum_p / len(stats), sum_r / len(stats), sum_f1 / len(stats)
    ...
    def eval(self, pred_result, use_name=False):
        ...
        for i in range(total):
            ...
            if pred_result[i] != neg:
                pred_positive += 1
            SentenceREDataset.update_rel_stat(pred_result[i], golden, stats, neg)
        acc = float(correct) / float(total)
        #gold - groud true relation name -> split target for relations
        try:
        ...
        try:
            ...
            micro_f1 = 0
        macro_p, macro_r, macro_f1 = SentenceREDataset.get_macro_stats(stats)
