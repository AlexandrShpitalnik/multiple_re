...
import tqdm
import random
from preprocess import prepare_cros_re_meta

rel_ids_set = {
    "RuMed" : {"Body_location_rel": 0, "Severity_rel": 1, "Course_rel": 2, "Modificator_rel": 3,
"Symptom_bdyloc_rel": 4},
    "GAD" : {"CID": 0}
}

ents_ids_set = {
    "GAD": {"Disease": "0", "Chemical":"1"}
}
            ...
            if (step + 1) == len(train_dataloader) - 1 or (
                ...
                if args.print_best_stat:
                    ...
                    if dev_score > best_dev_score:
                        ...
                        best_score, best_cnf, best_maco = evaluate(args, model, test_features, stat, not_found_rels, tag="dev")
                        if args.save_path != "":
...
def several_dirs_cv(args, config, tokenizer, aug_tokenizer, sent_tokenizer, rel_ids, ents_ids):
    ...
    f1_val, f1_macro, confusions_list = cv_mode(args, config, folds_dirs, tokenizer, aug_tokenizer, sent_tokenizer, rel_ids, ents_ids)
    return f1_val, f1_macro, confusions_list
...
def cv_mode(args, config, folds, tokenizer, aug_tokenizer, sent_tokenizer, rel_ids, ents_ids):
    ...
    for cur_dir in folds:
        ...
        cur_features_test, cur_not_proced_test = prepare_dataset(cur_dir, args, tokenizer, sent_tokenizer, rel_ids, ents_ids)
    ...
    for i in test_dirs:
        ...
        model = load_model(args, config, tokenizer)
...
def train_eval_mode(args, config, tokenizer, aug_tokenizer, sent_tokenizer, rel_ids, ents_ids):
    train_features_aug, not_found_rels_aug = prepare_dataset(args.train_file, args, aug_tokenizer, sent_tokenizer, rel_ids, ents_ids)
    train_features, not_found_rels = prepare_dataset(args.train_file, args, tokenizer, sent_tokenizer,
                                                             rel_ids, ents_ids)
    print(not_found_rels)
    test_features, not_found_rels_test = prepare_dataset(args.test_file, args, tokenizer, sent_tokenizer, rel_ids, ents_ids)
    print(not_found_rels_test)
    dev_features, not_found_rels_dev = prepare_dataset(args.dev_file, args, tokenizer, sent_tokenizer, rel_ids, ents_ids)
    dev_features_aug, not_found_rels_dev_aug = prepare_dataset(args.dev_file, args, aug_tokenizer, sent_tokenizer, rel_ids, ents_ids)
...
def main():
    ...
    # rel_ids = rel_ids_set['GAD']
    tokenizer = AutoTokenizer.from_pretrained(args.model_name_or_path)
    aug_tokenizer = AugmentingTokenizer(args.model_name_or_path)
    ents_ids, rel_ids = prepare_cros_re_meta(tokenizer)
